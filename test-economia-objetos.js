...
async function run() {
  ...
  // La publicación MÍA, no la primera que aparezca. Coger la primera
  // funcionaba solo porque el mercado estaba vacío; en cuanto había otra
  // de antes, se compraba sobre ella y el servidor contestaba "Cantidad
  // inválida" porque ya no le quedaban unidades. Que la prueba empiece
  // de cero lo arregla también, pero una prueba no debería depender de
  // eso para saber cuál es su propia publicación.
  const lista = await pedir('GET', '/api/market')
  const todas = ((lista.body || {}).listings || []).filter(l => l.itemId === 'iron_ore' && l.status === 'ACTIVE')
  const mio = todas.find(l => l.quantity >= 2) || todas[0]
...
if (process.argv.includes('--spawn')) {
  limpiarDatos('/tmp/cm-test-ecoobj.json', '/tmp/cm-ecoobj-b')
